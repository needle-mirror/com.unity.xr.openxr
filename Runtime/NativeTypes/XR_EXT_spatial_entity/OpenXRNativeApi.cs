...
namespace UnityEngine.XR.OpenXR.NativeTypes
{
    public static partial class OpenXRNativeApi
    {
        ...
        /// `xrCreateSpatialDiscoverySnapshotCompleteEXT` or `xrCreateSpatialUpdateSnapshotEXT`.</param>
        /// <param name="queryCondition">The query condition.</param>
        /// <param name="allocator">The allocation strategy to use for output arrays.</param>
        /// <param name="entityIds">The output entity IDs.</param>
        /// <param name="entityStates">The output entity states. Indices are parallel to
        /// <paramref name="entityIds"/></param>
        /// <returns>The result of the operation.\
        /// \
        /// Success codes:
        /// <list type="bullet">
        ...
        /// > Don't read the output if an error is returned.
        ///
        /// You are responsible to `Dispose` the output native arrays if you pass `Allocator.Persistent` as the
        /// <paramref name="allocator"/> value.
        /// </remarks>
        /// <exception cref="OverflowException">Thrown if either `entityIds.Length` or `entityStates.Length` would
        /// exceed <see cref="Int32.MaxValue"/>.</exception>
        public static unsafe XrResult xrQuerySpatialComponentDataEXT(
            XrSpatialSnapshotEXT snapshot,
            in XrSpatialComponentDataQueryConditionEXT queryCondition,
            Allocator allocator,
            out NativeArray<XrSpatialEntityIdEXT> entityIds,
            out NativeArray<XrSpatialEntityTrackingStateEXT> entityStates)
        {
            var queryResult = XrSpatialComponentDataQueryResultEXT.defaultValue;
            var result = xrQuerySpatialComponentDataEXT(snapshot, queryCondition, ref queryResult);
            if (result.IsError())
            {
                entityIds = default;
            ...
            entityIds = new NativeArray<XrSpatialEntityIdEXT>(checked((int)queryResult.entityIdCountOutput), allocator);
            entityStates = new NativeArray<XrSpatialEntityTrackingStateEXT>(
                checked((int)queryResult.entityStateCountOutput), allocator);
            ...
            return xrQuerySpatialComponentDataEXT(snapshot, queryCondition, ref queryResult);
        }
